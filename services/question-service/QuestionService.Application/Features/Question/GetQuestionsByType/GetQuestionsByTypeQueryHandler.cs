...
{
    public class GetQuestionsByTypeQueryHandler : IQueryHandler<GetQuestionsByTypeQuery, IEnumerable<QuestionDto>>
    {
        private readonly IQuestionRepository _questionRepository;
        private readonly IMapper _mapper;

        public GetQuestionsByTypeQueryHandler(IQuestionRepository questionRepository, IMapper mapper)
        {
            _questionRepository = questionRepository;
            _mapper = mapper;
        }

        public async Task<ApiResponse<IEnumerable<QuestionDto>>> Handle(GetQuestionsByTypeQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var questions = await _questionRepository.GetByQuestionTypeAsync(query.QuestionType);
